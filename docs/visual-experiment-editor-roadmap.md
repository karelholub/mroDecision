...
Status: in progress. DEE now exposes a draft-safe editor-session endpoint that returns a short-lived editor URL for a selected experiment, and the mock website supports `dee_editor=1` overlay mode with click-to-select, selector generation, selector-quality warnings, multi-change modification inventory, preview reset, live payload preview, and copyable structured `dom_modifications` JSON. The Experiment Design tab can import that copied payload into an editable draft treatment variant through the normal draft API, with a live payload review that shows target variant, modification inventory, selector warnings, and invalid guardrail issues before saving. DEE also has a short-lived editor-token save-back endpoint for website overlays; it can save guarded `dom_modifications` into a non-baseline draft variant without publishing. Wiring the overlay button, durable editor-token storage, variant management, and full undo/redo remain next.
