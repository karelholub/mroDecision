...
2. Add a native row-level managed-database adapter for enterprise deployments. The adapter registry, SQLite capability reporting, Postgres snapshot adapter, native Postgres schema contract, native migration runner, first read-only native store contract, settings/schema write contract, rule draft/version write contract, lookup/message write contract, message asset write contract, audit/client-event/assignment write contract, deployment-readiness diagnostics, and first async server wiring for readiness, evaluate/client evaluate, client feedback, rule versions, messages, lookups, schema, and message assets are implemented; the remaining work is the async evaluator/event-counter boundary, reporting endpoints, campaign operations, and final `postgres_native` adapter activation.
