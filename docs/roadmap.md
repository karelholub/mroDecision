...
1. Add deeper production prompt hardening and provider governance for the optional LLM-backed assistant. Provider latency/error observability, admin test connection, redacted config history, per-plan governance reports, prompt-contract metadata, and provider policy presets are implemented; durable per-call prompt/version audit trails remain.
