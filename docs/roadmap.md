...
6. Improve marketer setup flows: add form-based profile builders for Evaluate, reusable frequency-cap blocks, campaign folders, clearer campaign-level change logs, and bulk campaign operations. Evaluate Profile Builder, Frequency Cap Helper, campaign/folder fields, an Overview platform change log, campaign-level operational rollups, campaign drilldown, precompute missing-input diagnostics, guarded campaign bulk review/archive/duplicate actions, campaign/folder move actions, and asset-level move actions in rule/message inventories are implemented. Later: add multi-select "Add to campaign" bulk actions across inventory lists.

## Recommended Next Sprint

1. Finish the production-readiness slice with launch preflight, managed Postgres smoke validation, benchmark gates, Grafana/Prometheus import validation, and customer-staging evidence. See `docs/production-readiness-roadmap.md`.
2. Start the marketing UI refactor as the next major product workstream. The goal is to simplify DEE for marketing users while keeping it fast and navigable with large campaign/rule/message/experiment inventories. See `docs/marketing-ui-refactor-roadmap.md`.
3. First UI implementation slice: create a reusable scalable inventory component, apply it to Campaigns, convert Campaign detail into a page-level workbench, and add global quick search/open.
4. Add UI performance fixtures for 100, 1,000, and 10,000 campaigns/rules/messages/experiments before expanding the pattern to other workspaces.
